...
## It checks itself, once a day

Any command that is not itself about the database runs a health check first, at
most once per `check_interval_hours`. Three properties make it something you
leave switched on: findings go to **stderr** through the logger, so `scan
...
It looks at integrity, missing indexes, free pages, an oversized write-ahead log,
a pending migration, and any ceiling you declared. Each run appends to
...
This uses SQLite's online backup API, not a file copy: a copy taken mid-write
draws pages from two transactions and needs a WAL sidecar that will not match.
...
`prune` is the only thing here that deletes on purpose, so it is fenced four
ways:
...
The default scope is the command journal because that is operational volume — one
...
Every `[database]` key, with commentary: `vibe-sentinel scan --print-example`.
The ceilings default to **off**, because a number chosen inside a package cannot
know whether 200 MB is two years of healthy history or a runaway journal. The
...
Two write patterns share the file and take different durability trades: a scan
commits twice and keeps `synchronous=FULL`; the hook commits once per tool call
and takes `NORMAL`, because an fsync is about 7% of its whole budget and is spent
flushing a record of a command that has not run yet. Under WAL both are
corruption-safe; `NORMAL` gives up the last transactions to a machine power loss,
not to a process crash.

Adding a migration or an index: the checklists are in the module docstrings of
